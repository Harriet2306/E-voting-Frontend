...
const VerificationPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-primary-light via-surface to-primary-light relative overflow-hidden">
      {/* Animated background elements */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-40 -right-40 w-80 h-80 bg-primary/30 rounded-full mix-blend-multiply filter blur-xl opacity-20 animate-float"></div>
      ...
      <div className="flex items-center justify-center min-h-screen py-8 sm:py-12 px-4 relative z-10">
        <Card className="w-full max-w-md shadow-2xl border-2 border-card-stroke bg-surface rounded-2xl sm:rounded-3xl animate-bounce-in transform transition-all duration-300 hover:shadow-primary/20 hover:scale-[1.02]">
          <CardHeader className="space-y-2 sm:space-y-3 text-center pb-4 sm:pb-6 px-4 sm:px-6 pt-6 sm:pt-8">
            <div className="mx-auto flex h-16 w-16 sm:h-20 sm:w-20 items-center justify-center rounded-2xl sm:rounded-3xl bg-gradient-to-br from-primary to-primary-dark text-primary-foreground shadow-xl animate-float hover:scale-110 transition-transform duration-300 cursor-pointer">
            ...
            {step === 'request' ? (
              <form onSubmit={requestOTPForm.handleSubmit(onRequestOTP)} className="space-y-4 sm:space-y-6">
                <div className="space-y-2 animate-fade-in-up" style={{ animationDelay: '0.4s', animationFillMode: 'both' }}>
                  <Label htmlFor="regNo" className="text-sm font-semibold text-foreground">Registration Number</Label>
                  <Input
                    id="regNo"
                    type="text"
                  ...
                  {requestOTPForm.formState.errors.regNo && (
                    <p id="regNo-error" className="text-sm text-error animate-slide-in-right">
                ...
                <div className="p-4 bg-primary-light border-2 border-card-stroke rounded-xl animate-fade-in-up" style={{ animationDelay: '0.6s', animationFillMode: 'both' }}>
                  <p className="text-xs text-primary-dark leading-relaxed">
                    <strong>Note:</strong> OTP will be sent to your email address. The code expires in <strong>5 minutes</strong>.
                  </p>
                </div>
              </form>
            ) : (
              <form onSubmit={verifyOTPForm.handleSubmit(onVerifyOTP)} className="space-y-6">
                {otpSent && (
                  <div className="p-4 bg-primary-light border-2 border-card-stroke rounded-xl animate-slide-in-right">
                    <div className="flex items-start gap-3">
                      <svg className="h-5 w-5 text-primary mt-0.5 flex-shrink-0 animate-bounce-slow" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="space-y-2 animate-fade-in-up" style={{ animationDelay: '0.4s', animationFillMode: 'both' }}>
                  <Label htmlFor="otp" className="text-sm font-semibold text-foreground">Enter 6-Digit OTP</Label>
                  <Input
                    id="otp"
                    type="text"
                    placeholder="000000"
                  ...
                  {verifyOTPForm.formState.errors.otp && (
                    <p id="otp-error" className="text-sm text-error animate-slide-in-right">
                ...
                <Button
                  type="button"
                  variant="ghost"
                  className="w-full text-sm text-muted-foreground hover:text-foreground transition-all duration-200 hover:scale-105"
                  onClick={async () => {
                    try {
