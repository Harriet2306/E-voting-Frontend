...
const Sidebar: React.FC<SidebarProps> = ({ role, isOpen, setIsOpen, onNavAction }) => {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'fixed left-0 top-0 h-full z-50 bg-surface border-r border-card-stroke shadow-sm transition-all duration-300 ease-in-out',
          isOpen ? 'w-64' : 'w-20'
        )}
      >
        {/* Logo Section */}
        <div className="flex items-center justify-between p-4 border-b border-card-stroke h-16 bg-primary-light/30">
          {isOpen ? (
            <div className="flex items-center gap-3 min-w-0 flex-1">
              <Logo size="sm" showText={false} />
              <div className="min-w-0">
                <h2 className="font-bold text-lg text-primary truncate">
                  BALLOT BUDDY
                </h2>
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  active
                    ? 'bg-primary-light text-primary-dark shadow-sm border border-card-stroke'
                    : 'text-foreground hover:bg-primary-light/50 hover:text-primary border-transparent',
              ...
              >
                ...
                {isOpen && (
                  <>
                    ...
                    {item.badge && (
                      <span className="px-2 py-1 text-xs font-bold rounded-full bg-primary-light text-primary-dark">
                        {item.badge}
                      </span>
                    )}
                    {active && (
                ...
                {!isOpen && (
                  <div className="absolute left-full ml-2 px-3 py-2 bg-foreground text-surface text-sm rounded-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-50">
                    {item.label}
                  </div>
                )}
              </button>
